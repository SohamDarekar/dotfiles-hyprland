...
if (
    ...
    and data.BAR_POSITION in ["Left", "Right"]
        ...
        if self.content is None:
            ...
            if self.content_type == "file":
                ...
                if is_url(self.content):
                    ...
                    download_favicon(
                        self.content,
                        lambda path: self.update_favicon(
                            icon_container, url_icon, path
                        ),
                    )
                else:
        ...
        if content_type == "inode/directory":
            try:
                ...
                return Gtk.Image.new_from_icon_name(
                    "default-folder", Gtk.IconSize.DIALOG
                )
...
class Pins(Gtk.Box):
            ...
            if cell.content_type == "file" and cell.content:
                ...
                if os.path.exists(dir_path) and dir_path not in self.monitored_paths:
                    self.observer.schedule(
                        self.event_handler, dir_path, recursive=False
                    )
    ...
    def stop_monitoring(self):
        ...
        for cell in self.cells:
            if (
                hasattr(cell, "favicon_temp_path")
