...
class VolumeIcon(Box):
    ...
    def _check_device_reconnection(self):
        """Check for audio device reconnection and restart audio if needed."""
        try:
            # Check if we lost audio device but media should be playing
            if not self.audio.speaker and self._was_playing_media():
                # Try to refresh audio system
                GLib.timeout_add(1000, self._restart_audio_if_needed)
        except Exception as e:
            logger.warning(f"Device reconnection check failed: {e}")
        return True

    def _was_playing_media(self):
        """Check if media was recently playing."""
        try:
            import subprocess
            # Check for running media players
            result = subprocess.run(['pgrep', '-f', 'spotify|firefox|chromium|vlc|mpv'], 
                                  capture_output=True, text=True)
            return result.returncode == 0
        except:
            return False

    def _restart_audio_if_needed(self):
        """Restart audio subsystem if device reconnected."""
        try:
            # Restart PulseAudio to recognize reconnected devices
            exec_shell_command_async("systemctl --user restart pipewire-pulse")
            print("Restarted audio subsystem for device reconnection")
        except Exception as e:
            print(f"Failed to restart audio subsystem: {e}")
        return False
...
class MicIcon(Box):
    ...
    def on_microphone_changed(self, *_):
        if not self.audio.microphone:
            self.mic_button.get_child().set_markup(icons.mic)
            self.remove_style_class("muted")
            self.set_tooltip_text("No microphone")
            return
...
class ControlSmall(Box):
    ...
    def destroy(self):
        if self._update_source_id is not None:
            GLib.source_remove(self._update_source_id)
        if self._device_monitor_id is not None:
            GLib.source_remove(self._device_monitor_id)
        super().destroy()
