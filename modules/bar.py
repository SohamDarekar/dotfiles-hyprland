...
class WorkspaceManager:
    """
    Tracks the current dynamic list of workspaces and notifies listeners on changes.
    """
    ...
    def __init__(self):
        ...
        self.workspaces = []
        self.listeners = []
        self._init_from_hyprland()
        # Listen for workspace events
        self.conn.connect("event::workspace", self._on_workspace_event)
        self.conn.connect("event::openwindow", self._on_window_event)
        self.conn.connect("event::closewindow", self._on_window_event)

    def _init_from_hyprland(self):
        # Query current workspaces from Hyprland
        try:
            import json

            ws_data = json.loads(self.conn.send_command("j/workspaces").reply.decode())
            # Instead of limiting to 3, include all workspace IDs found
            self.workspaces = sorted([ws["id"] for ws in ws_data])
            if not self.workspaces:
                self.workspaces = [1]
        except Exception:
            self.workspaces = [1]

    def _on_workspace_event(self, *args):
        self._update_from_hyprland()

    def _on_window_event(self, *args):
        self._update_from_hyprland()

    def _update_from_hyprland(self):
        old_ws = set(self.workspaces)
        self._init_from_hyprland()
        if set(self.workspaces) != old_ws:
    ...
    def add_workspace(self):
        # Find the next unused workspace id (no upper limit)
        next_id = 1
        while next_id in self.workspaces:
            next_id += 1
        self.conn.send_command(f"/dispatch workspace {next_id}")

    def remove_workspace(self, ws_id):
        # Only remove if more than one workspace remains
        if len(self.workspaces) <= 1:
            return
        self.conn.send_command(
            f"/dispatch movetoworkspacesilent 1,address:$(hyprctl activewindow -j | jq -r .address)"
        )
        self.conn.send_command(f"/dispatch workspace {ws_id}")
        self.conn.send_command(f"/dispatch killworkspace {ws_id}")

    def get_workspaces(self):
        # Return all workspaces, no limit
...
class Bar(Window):
    def __init__(self, **kwargs):
        ...
        self.workspaces_num = None
        self.ws_container = None

        # Workspace UI is now dynamic
        self._build_workspace_ui()
        self.workspace_manager.add_listener(self._on_workspaces_changed)
    ...
    def _build_workspace_ui(self):
        # Build workspace buttons dynamically
        def make_buttons(ws_ids, use_chinese=False):
            return [
                WorkspaceButton(
                    h_expand=False,
                    v_expand=False,
                    ...
                    id=ws_id,
                    label=(
                        CHINESE_NUMERALS[ws_id - 1]
                        if use_chinese and 1 <= ws_id <= len(CHINESE_NUMERALS)
                        else str(ws_id)
                    ),
        ...
        ws_ids = self.workspace_manager.get_workspaces()
        self.workspace_buttons = make_buttons(ws_ids, use_chinese=False)
        self.workspace_buttons_num = make_buttons(
            ws_ids, use_chinese=data.BAR_WORKSPACE_USE_CHINESE_NUMERALS
        )
    ...
    def _on_workspaces_changed(self, ws_ids):
        # Rebuild workspace buttons and update UI
        def make_buttons(ws_ids, use_chinese=False):
            return [
                WorkspaceButton(
                    h_expand=False,
        ...
        self.workspaces.buttons = self.workspace_buttons
        self.workspaces_num.buttons = self.workspace_buttons_num
        self.ws_container.children = (
            self.workspaces
            if not data.BAR_WORKSPACE_SHOW_NUMBER
            else self.workspaces_num
        )
