2# Thanks to https://github.com/muhchaudhary for the original code. You are a legend.
import json
...
class Overview(Box):
    ...
    def _init_workspace_manager(self):
        # Import WorkspaceManager from bar.py or define a shared one
        from modules.bar import WorkspaceManager
        self.workspace_manager = WorkspaceManager()
        self.workspace_manager.add_listener(self._on_workspaces_changed)

    def _on_workspaces_changed(self, ws_ids):
        self.update(signal_update=True, ws_ids=ws_ids)
    ...
    def update(self, signal_update=False, ws_ids=None):
        self._all_apps = get_desktop_applications()
        self.app_identifiers = self._build_app_identifiers_map()
        ...
        # Use dynamic workspace list
        if ws_ids is None:
            ws_ids = self.workspace_manager.get_workspaces()

        # Determine grid size based on workspace count
        num_ws = len(ws_ids)
        ...
        # Build dynamic grid for workspaces
        for idx, w_id in enumerate(ws_ids):
            if rows == 2:
        ...
        # Add "add workspace" button if allowed
        if len(ws_ids) < getattr(data, "MAX_WORKSPACES", 20):
            add_row = self.children[-1]
            add_row.add(
                Box(
                    name="overview-workspace-box",
                    orientation="vertical",
                    children=[
                        Label(name="overview-workspace-label", label="Add Workspace"),
                        WorkspaceEventBox(
                            0,  # Special id for add
                            None
                        ),
                    ],
                )
...
# Patch WorkspaceEventBox to handle add/remove workspace
class WorkspaceEventBox(EventBox):
    def __init__(self, workspace_id: int, fixed: Gtk.Fixed | None = None):
        ...
        super().__init__(
            ...
            if fixed
                ...
                h_expand=True,
                v_expand=True,
                markup=icons.circle_plus if workspace_id == 0 else "",
            ),
            ...
            ) if workspace_id > 0 else None,
            on_button_press_event=lambda _w, event: self._on_add_workspace(event) if workspace_id == 0 and event.button == 1 else None,
        )
        self.drag_dest_set(
    ...
    def _on_add_workspace(self, event):
